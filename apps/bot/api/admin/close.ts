...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    if (!event) {
      res.status(400).json({ message: "Event must be in published status" });
      return;
    }

    res.status(200).json({ event });
  } catch (error) {
    logError("admin_close_failed", { error, eventId });
    res.status(500).json({ message: "Failed to close event" });
  }
}
