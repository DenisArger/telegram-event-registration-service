...
import { createServiceClient, getEventById, getUserById, promoteNextFromWaitlist } from "@event/db";
  ...
  try {
    ...
    if (result.status === "promoted" && result.user_id) {
      ...
      if (promotedUser?.telegramId) {
        notifyTasks.push(sendTelegramMessage(promotedUser.telegramId, participantMessage, process.env));
      }

      if (ctx.principal.telegramId) {
        notifyTasks.push(sendTelegramMessage(ctx.principal.telegramId, adminMessage, process.env));
      }
